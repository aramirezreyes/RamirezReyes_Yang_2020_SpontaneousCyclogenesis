...
"""
function getapeanalysis_with_condensate_loading(file2d,file3d,outputfile,initial_timeindex,final_timeindex,partnumber,outputInterval,position,FloatType::Type=Float64;tropopause_height = 15000)
    ...
    @info "Read 60%"
    Condensate          = variable(ds3d,"QP")[:,:,:,iterator_time_3d] :: Array{Float32,4}
    Condensate          .= Condensate .+ variable(ds3d,"QN")[:,:,:,iterator_time_3d] :: Array{Float32,4}
    @info "Read 60%"    
    ...
    fCondensate = @spawnat 7 cutborders!(filter_array(Condensate,smooth_x,smooth_time,1),smooth_time,position)
    ...
    B            = g .* (var_ThetaV./xBar_ThetaV .- 1e-3Condensate)
...
function computebudgets_with_condensate(exp_name)
    ...
    outputfile = string("/global/cscratch1/sd/aramreye/for_postprocessing/ApeBudgetOutputs2/",exp_name,"_with_condensate")
    ...
    for part_number in 1:number_of_parts
        ...
        flush(stdout)
        flush(stderr)
        getapeanalysis_with_condensate_loading(file2d,file3d,outputfile,initial_timestep,final_timestep,part_number,output_timestep,position,Float32)

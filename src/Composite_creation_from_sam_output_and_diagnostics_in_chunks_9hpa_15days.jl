...
function composite_and_radialaverage_in_chunks!(azimuthal_average,composite_addition,buf1,buf2,radiusbins,variable_name,pre_iterator_time,nchunks,file_data,diagnostics_data_dir,exp_name,centers_labels_and_cyclones;maskcyclones = true)
    ...
    for chunk in 1:nchunks
        iterator_time_2d = (2402 + (chunk - 1)*chunk_size_2d): 2 : (2402 + chunk*chunk_size_2d - 1)
        iterator_time_data =  ((chunk - 1)*chunk_size_3d + 1 ) : (chunk*chunk_size_3d)
        iterator_time_3d = 1100 .+ iterator_time_data
        ...
        #centers_labels_and_cyclones = jldopen(file_centers) do file              
        #    read(file,"centers_data")
        #end
        
        #@info nchunks size(variable_of_interest) #size(centers_labels_and_cyclones)
...
function gather_diagnostic_3d_in_chunks(data_dir,exp_name,desired_var,chunk,chunksize,nchunks)
    ...
    for iter in 1:howmanyfiles
        ...
        file_name = string(exp_name,"_nosmoothing",filenumber+10,".jld")
...
function get_composites_in_chunks(file2d,file3d,exp_name)
    ...
    composites_data_dir = "/global/cscratch1/sd/aramreye/for_postprocessing/CompositeOutputs_50d_9hpa_10days/"
    azimuthal_averages_data_dir = "/global/cscratch1/sd/aramreye/for_postprocessing/AzimuthualAverageOutputs_50d_9hpa_10days/"
    ...
    centers_labels_and_cyclones = @views [
        detect_cyclones!(buf_2d_1, buf_2d_2, pressure_anomaly[:,:,timeindex],-9,2000) for
...
#    nchunks=6   ## this is for 50 days!
    ...
    #create files:
    ds =  jldopen(composite_filename_withmask, "w") do file end
    ds =  jldopen(composite_filename_nomask, "w") do file end
    ds =  jldopen(azimuthal_filename_withmask, "w") do file end
    ...
    #     ## Create products with mask
        ...
        composite_and_radialaverage_in_chunks!(azimuthal_average_3d,composite_addition_3d,buf_3d_1,buf_3d_2,radiusbins,current_variable,pre_iterator_time_3d,nchunks,file3d,diagnostics_data_dir,exp_name,centers_labels_and_cyclones;maskcyclones = true)
    ...
    for current_variable in variables_orig_2d
        ...
        composite_and_radialaverage_in_chunks!(azimuthal_average_2d,composite_addition_2d,buf_2d_1,buf_2d_2,radiusbins,current_variable,pre_iterator_time_3d,nchunks,file2d,diagnostics_data_dir,exp_name,centers_labels_and_cyclones;maskcyclones = true)
         @info "Writing"
         flush(stdout)
         jldopen(composite_filename_withmask, "r+",mmaparrays=true) do file
